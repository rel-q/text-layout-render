...
public:
    ...
    inline TextureRect(float left, float top, float right, float bottom) :
            left(left),
            top(top),
            right(right),
            bottom(bottom) {
    ...
    inline TextureRect(float width, float height) :
            left(0.0f),
            top(0.0f),
            right(width),
            bottom(height) {
    ...
    friend int operator==(const TextureRect& a, const TextureRect& b) {
    ...
    inline void set(const TextureRect& r) {
    ...
    bool intersects(const TextureRect& r) const {
    ...
    void doIntersect(const TextureRect& r) {
    ...
    inline bool contains(const TextureRect& r) const {
        return contains(r.left, r.top, r.right, r.bottom);
    }

    bool unionWith(const TextureRect& r) {
    ...
    void snapGeometryToPixelBoundaries(bool snapOut) {
        if (snapOut) {
             ...
             * bounds by a fudge factor. This ensures that ambiguous geometry (e.g. a non-AA TextureRect
     ...
     */
    void expandToCover(const TextureRect& other) {
    ...
    void dump(const char* label = nullptr) const {
        printf("%s[l=%.2f t=%.2f r=%.2f b=%.2f] \n", label ? label : "TextureRect", left, top, right, bottom);
    }

    friend std::ostream& operator<<(std::ostream& os, const TextureRect& rect) {
        if (rect.isEmpty()) {
