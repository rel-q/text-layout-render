...
struct CacheBlock {
    ...
    void output() {
        ...
        while (currBlock) {
            printf("Block: this, x, y, w, h = %p, %d, %d, %d, %d \n",
...
public:
    ...
    bool fitBitmap(const GlyphInfo& glyph, uint32_t* retOriginX, uint32_t* retOriginY);
    ...
    inline const TextureRect* getDirtyRect() const {
        return &mDirtyRect;
    }
