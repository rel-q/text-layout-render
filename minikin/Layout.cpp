...
class LayoutCacheKey {
 public:
  ...
  hash_t hash() const { return mHash; }
...
hash_t LayoutCacheKey::computeHash() const {
  ...
  hash = JenkinsHashMix(hash, hash_type(mHyphenEdit.getHyphen()));
...
BidiText::BidiText(const uint16_t* buf,
  ...
  mBidi = ubidi_open();
  if (!mBidi) {
    printf("error creating bidi object \n");
    return;
  }
  ...
  ubidi_setClassCallback(mBidi, emojiBidiOverride, nullptr, nullptr, nullptr,
                         &status);
  if (!U_SUCCESS(status)) {
    printf("error setting bidi callback function, status = %d \n", status);
    return;
  }
  ...
  ubidi_setPara(mBidi, reinterpret_cast<const UChar*>(buf), mBufSize, bidiReq,
                NULL, &status);
  if (!U_SUCCESS(status)) {
    printf("error calling ubidi_setPara, status = %d \n", status);
    return;
  }
  ...
  if (!U_SUCCESS(status) || rc < 0) {
    printf("error counting bidi runs, status = %d \n", status);
  }
...
void Layout::doLayoutRun(const uint16_t* buf,
  ...
  for (int run_ix = isRtl ? items.size() - 1 : 0;
    ...
    if (run.fakedFont.font == NULL) {
      printf("no font for run starting u+%04x length %d \n", buf[run.start],
            run.end - run.start);
      continue;
    }
    ...
    for (ssize_t scriptRunStart = run.start; scriptRunStart < run.end;
      ...
      for (unsigned int i = 0; i < numGlyphs; i++) {
        ...
        if (static_cast<size_t>(info[i].cluster - clusterOffset) < count) {
          ...
          printf("cluster %zu (start %zu) out of bounds of count %zu \n",
