...
static size_t toLanguageTag(char* output,
  ...
  if (U_FAILURE(uErr)) {
    ...
    printf("uloc_canonicalize(\"%s\") failed: %s \n", locale.c_str(),
          u_errorName(uErr));
    output[0] = '\0';
    return 0;
  }
  ...
  if (U_FAILURE(uErr)) {
    // unable to build a proper language identifier
    printf("uloc_addLikelySubtags(\"%s\") failed: %s \n", output,
          u_errorName(uErr));
    output[0] = '\0';
    return 0;
  }
  ...
  if (U_FAILURE(uErr)) {
    // unable to build a proper language identifier
    printf("uloc_toLanguageTag(\"%s\") failed: %s \n", likelyChars,
          u_errorName(uErr));
    output[0] = '\0';
    return 0;
  }
#ifdef VERBOSE_DEBUG
  printf("ICU normalized '%s' to '%s'", locale.c_str(), output);
#endif
  return outLength;
}
